export const config = {
  runtime: "edge",
  regions: ["fra1", "cdg1", "lhr1"], // EU (можно оставить только fra1)
};

const MODEL = "models/gemini-flash-lite-latest";
const GEMINI_TIMEOUT_MS = 9000;     // 9 секунд
const MAX_OUTPUT_TOKENS = 220;      // меньше = быстрее и меньше 504
...
export default async function handler(request) {
  ...
  try {
    ...
    if (!r.ok) {
      // ВАЖНО: всегда возвращаем JSON c status
